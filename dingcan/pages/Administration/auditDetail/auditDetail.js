...
Page({
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          wx.request({
            url: app.globalData.baseUrl + "/applet/meal/manager/close_order",
            method: "POST",
            header: {
    ...
    wx.showModal({
      ...
      success: function (res) {
        if (res.confirm) {
          ...
          wx.request({
            url: app.globalData.baseUrl + "/applet/meal/manager/audit_order",
            method: "POST",
            header: {
              'token': app.globalData.token
            },
  ...
  getDetail() {
    ...
    let url = '/applet/meal/detail_order'
    wx.showToast({
      title: '加载中',
      icon: 'loading',
      duration: 10000
    ...
    wx.request({
      ...
      success: function (res) {
        console.log('订餐详情', res.data.data)
        wx.hideToast();
        that.setData({
  ...
  downLoadFile() {
    ...
    console.log('downloadFile', that.data.baseUrl + that.data.detail.work.extra_file)
    let fileUrl = that.data.baseUrl + that.data.detail.work.extra_file
    ...
    if (fileType==='jpg' || fileType=='png'||fileType=='gif') {
      ...
      wx.downloadFile({
        url: that.data.baseUrl + that.data.detail.work.extra_file,
        success: function (res) {
